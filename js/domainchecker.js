...
jQuery("#btnCheckAvailability").click(function(){
	try {
		if ($('#inputDomain').length) {
			var convertedDomain = punycode.ToUnicode(jQuery("#inputDomain").val());
			jQuery("#inputDomain").val(convertedDomain);
		}
		else {
			var lines = jQuery('#inputBulkDomains').val().split('\n');
			
			var newTextareaValue = '';
			for(var i = 0;i < lines.length;i++){
			    var convertedDomain = punycode.ToUnicode(lines[i]);
				newTextareaValue += convertedDomain + "\n";
			}
			
			jQuery("#inputBulkDomains").val(newTextareaValue);
		}
